...
var vocab = [    //array of sentences, note that part1 ends with space and part2 starts with a space
    ...
    {part1:"Joshua's stomach felt ", part2:" after drinking expired milk.", w1:"funny", w2:"tummy", w3:"spilled", correct:"funny", explanation:"The correct answer is 'funny'. If your stomach feels funny, it means it feels strange. Tummy is a synonym for stomach, and spilled is a verb."},
...
var grammar = [
    ...
    {part1:"The cat was licking ", part2:" tail.", w1:"it is", w2:"it", w3:"its", correct:"its", explanation: "The correct answer is 'its'. The tail is the cat's tail so we need a possessive 'its'."},
...
function clearSents() {//this function only clears sentences in localStorage, it's written here for convenience
    if(confirm("Are you sure you want to delete all custom sentences?\nThis cannot be undone")) {
        localStorage.removeItem("customSents");
        alert("sentences cleared");
    }
}

function saveSent() {
    if(localStorage.getItem("customSents") == null) {
        var temp = [];
    }else{
        var temp = JSON.parse( localStorage.getItem("customSents") )
    }
    
    temp.push({part1:document.getElementById("part1").value, part2:document.getElementById("part2").value, 
                   w1:document.getElementById("w1").value, w2:document.getElementById("w2").value, w3:document.getElementById("w3").value, correct:document.getElementById("correct").value,
                    explanation:document.getElementById("explanation").value });
    localStorage.setItem("customSents", JSON.stringify(temp));
    window.location.href = "teachingtools.html";
}
